...
if secao == "Visão Geral":
    ...
    with tab_layout:
        ...
        # ── Diagrama Plotly — fiel ao layout das colegas ──────────────────────
        st.markdown("**Diagrama do layout (proporcional ao Excel)**")

        # Normalizar para [0,1]×[0,1] usando proporções reais
        # Eixo X = comprimento (U=69m), Eixo Y = largura (V=29m), Y=0 em baixo
        # Zona racks: colunas de prateleiras na parte esquerda/superior
        # Zona D: parte inferior + direita

        # Proporções em x e y
        # Racks ocupam ~70% do comprimento e toda a largura menos a zona D inferior
        # Zona D inferior: Receção+Expedição = 16m+20m = 36m de largura, 29m comprimento
        # Zona D direita: Escritórios+Picking = 11.5m+36.3m comprimento, 20m largura

        # Em coordenadas normalizadas [0,1]:
        # Y: 0=baixo, 1=cima
        # Zona D inferior: y=0 → y= 20/29 = 0.69 (Recep 16m + Exped 20m max=20m)
        # Zona D direita (Escritórios+Picking): x= (69-36.3)/69=0.47 → x=1, y=20/29→1
        # Zona racks: x=0→0.47, y=0.69→1  (aproximação proporcional)

        x_div = (U - ZONAS_D["Picking / Preparação"]["comp"]) / U   # ~0.474
        y_div = ZONAS_D["Área de Expedição"]["larg"] / V              # ~0.69

        fig_lay = go.Figure()

        # Zona de racks (azul)
        fig_lay.add_shape(type="rect", x0=0, y0=y_div, x1=x_div, y1=1.0,
                          fillcolor="rgba(30,58,95,0.45)", line=dict(color="#2563eb", width=2))
        fig_lay.add_annotation(x=x_div/2, y=(y_div+1)/2+0.05,
                                text="<b>Zona de Armazenamento</b>",
                                showarrow=False, font=dict(color="#60a5fa", size=12), xanchor="center")
        fig_lay.add_annotation(x=x_div/2, y=(y_div+1)/2-0.05,
                                text=f"{AREA_RACKS:.0f} m²  ·  racks EUR1 triplos  ·  4 níveis",
                                showarrow=False, font=dict(color="#60a5fa", size=9), xanchor="center")

        # Racks simbolicos (linhas verticais)
        n_filas = 9
        for fi in range(n_filas):
            rx = 0.03 + fi * (x_div - 0.03) / n_filas
            fig_lay.add_shape(type="rect",
                              x0=rx, y0=y_div+0.03, x1=rx+0.04, y1=0.97,
                              fillcolor="rgba(30,58,95,0.8)",
                              line=dict(color="rgba(96,165,250,0.4)", width=1))

        # Escritórios (laranja claro) — canto superior direito
        x_escrit = x_div
        y_escrit = y_div + ZONAS_D["Escritórios e Áreas Sociais"]["larg"] / V * (1-y_div) / (ZONAS_D["Área de Expedição"]["larg"]/V)
        # Simplificar: escritórios ocupam parte superior da zona D direita
        y_escrit_bot = y_div + (1-y_div)*0.45
        fig_lay.add_shape(type="rect", x0=x_div, y0=y_escrit_bot, x1=1.0, y1=1.0,
                          fillcolor="rgba(251,191,36,0.3)", line=dict(color="#f59e0b", width=2))
        fig_lay.add_annotation(x=(x_div+1)/2, y=(y_escrit_bot+1)/2,
                                text="<b>Escritórios</b>",
                                showarrow=False, font=dict(color="#f59e0b", size=11), xanchor="center")
        fig_lay.add_annotation(x=(x_div+1)/2, y=(y_escrit_bot+1)/2-0.07,
                                text=f"{ZONAS_D['Escritórios e Áreas Sociais']['area']} m²",
                                showarrow=False, font=dict(color="#f59e0b", size=9), xanchor="center")

        # Zona de Preparação/Picking (rosa/roxo) — parte inferior da zona D direita
        fig_lay.add_shape(type="rect", x0=x_div, y0=y_div, x1=1.0, y1=y_escrit_bot,
                          fillcolor="rgba(168,85,247,0.3)", line=dict(color="#a855f7", width=2))
        fig_lay.add_annotation(x=(x_div+1)/2, y=(y_div+y_escrit_bot)/2,
                                text="<b>Zona de Preparação</b>",
                                showarrow=False, font=dict(color="#c084fc", size=11), xanchor="center")
        fig_lay.add_annotation(x=(x_div+1)/2, y=(y_div+y_escrit_bot)/2-0.07,
                                text=f"{ZONAS_D['Picking / Preparação']['area']} m²",
                                showarrow=False, font=dict(color="#c084fc", size=9), xanchor="center")

        # Receção e Conferência (verde) — parte inferior esquerda
        x_recep = ZONAS_D["Área de Receção e Conferência"]["comp"] / U  # 29/69=0.42
        fig_lay.add_shape(type="rect", x0=0, y0=0, x1=x_recep, y1=y_div,
                          fillcolor="rgba(16,185,129,0.3)", line=dict(color="#10b981", width=2))
        fig_lay.add_annotation(x=x_recep/2, y=y_div/2+0.04,
                                text="<b>Área de Receção</b>",
                                showarrow=False, font=dict(color="#34d399", size=10), xanchor="center")
        fig_lay.add_annotation(x=x_recep/2, y=y_div/2-0.04,
                                text=f"<b>e Conferência</b>",
                                showarrow=False, font=dict(color="#34d399", size=10), xanchor="center")
        fig_lay.add_annotation(x=x_recep/2, y=y_div/2-0.12,
                                text=f"{ZONAS_D['Área de Receção e Conferência']['area']} m²",
                                showarrow=False, font=dict(color="#34d399", size=9), xanchor="center")

        # Expedição (azul claro) — parte inferior direita
        fig_lay.add_shape(type="rect", x0=x_recep, y0=0, x1=1.0, y1=y_div,
                          fillcolor="rgba(14,165,233,0.3)", line=dict(color="#0ea5e9", width=2))
        fig_lay.add_annotation(x=(x_recep+1)/2, y=y_div/2,
                                text="<b>Área de Expedição</b>",
                                showarrow=False, font=dict(color="#38bdf8", size=11), xanchor="center")
        fig_lay.add_annotation(x=(x_recep+1)/2, y=y_div/2-0.08,
                                text=f"{ZONAS_D['Área de Expedição']['area']} m²",
                                showarrow=False, font=dict(color="#38bdf8", size=9), xanchor="center")

        # Docas na fronteira Receção/Expedição — em baixo
        for di in range(N_DOCAS):
            dx = 0.15 + di * 0.25
            fig_lay.add_shape(type="rect", x0=dx, y0=-0.04, x1=dx+0.12, y1=0.04,
                              fillcolor="rgba(22,163,74,0.5)", line=dict(color="#4ade80", width=2))
            fig_lay.add_annotation(x=dx+0.06, y=0,
                                   text=f"<b>D{di+1}</b>",
                                   showarrow=False, font=dict(color="#f1f5f9", size=9), xanchor="center")

        # Seta Fluxo U
        fig_lay.add_annotation(x=0.75, y=0.92, text="↓ Receção",
                                showarrow=False, font=dict(color="#94a3b8", size=9))
        fig_lay.add_annotation(x=0.75, y=0.55, text="↕ Armazenamento",
                                showarrow=False, font=dict(color="#94a3b8", size=9))
        fig_lay.add_annotation(x=0.75, y=0.15, text="↑ Expedição  (Fluxo U)",
                                showarrow=False, font=dict(color="#94a3b8", size=9))
